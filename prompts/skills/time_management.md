---
version: 1.1.0
author: gg (v10.2 evolution)
description: "统一的时间管理系统，确保所有技能和工作流中的时间处理一致性和准确性。已修正时间获取机制。"
...
4. **实时获取**: 每次需要时间信息时都必须重新执行 `date` 命令，不能缓存或假设时间
5. **时区处理**: 确保时间获取考虑了正确的时区设置（当前为CST中国标准时间）
