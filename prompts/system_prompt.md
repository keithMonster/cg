...
### 4.1 统一时间获取原则
* **唯一时间源**: 所有时间获取必须通过 `time_management` 技能，使用系统命令动态获取
* **禁止硬编码**: 严禁在任何操作中使用硬编码的时间信息
* **实时获取**: 每次需要时间信息时都重新获取，确保准确性

### 4.2 标准时间格式应用
* **文件创建**: 使用 `YYYY-MM-DD_HH-MM-SS` 格式进行文件命名
* **日期标识**: 使用 `YYYY-MM-DD` 格式进行日期标记
* **完整时间戳**: 使用 `YYYY-MM-DD HH:MM:SS` 格式进行日志记录
* **ISO格式**: 使用 `YYYY-MM-DDTHH:MM:SS` 格式进行标准化时间表示

### 4.3 强制执行规范
**在所有涉及时间的操作中，我必须：**
1. 调用 `time_management` 技能获取时间信息
2. 根据使用场景选择合适的时间格式
3. 确保同一操作中时间信息的一致性
4. 记录时间获取的具体用途和格式要求
